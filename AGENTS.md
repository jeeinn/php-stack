...
- **核心功能**:
  ...
  - 打包 `.env`、`docker-compose.yml`、`services/` 配置、`.user-config/`（镜像源/版本覆盖/站点等；不含备份页 UI 偏好 `backup.json`）
...
- **核心功能**:
  ...
  - 配置文件、项目文件、SQL 文件还原（SQL **导入执行**待完善）
  - 站点宿主机路径跨机覆写
  - 恢复前自动回滚包 + 明细结果
  - 进度通知与错误汇总
...
✅ **环境恢复** - 完整实现需求 4.1-4.10（含预览端口冲突检测、站点路径覆写、回滚包）
✅ **前端国际化（i18n）** - 中/英双语，运行时切换
✅ **主题预设** - 自动/明亮/暗黑三种模式
✅ **工作区与版本管理** - 多工作区、版本清单、override/custom
