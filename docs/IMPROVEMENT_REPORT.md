...
### R1.【P0·安全】恢复引擎存在 Zip-Slip 路径遍历风险
...
### R2.【P0·可靠】恢复会直接覆盖现有文件，且无恢复前自动备份
...
### R3.【P1·可靠】Docker 启动流程在 async 命令中执行阻塞 IO
...
### R4.【P1·可靠】容器状态用 `format!("{:?}")` 字符串作为前后端契约
...
### A1.【P0】路径解析逻辑重复三份，且生产模式写到 exe 同级目录
...
### U1.【P0·功能缺陷】多个 PHP 服务时自定义扩展输入互相污染
...
### U2.【P1】恢复结果只有"成功/失败"一句话，错误明细不可读
...
### U4.【P1】UI 日志与文件日志是两套东西，复制按钮语义混乱
...
### U6.【P2】WorkspaceInitDialog 不适配亮色主题
...
### U7.【P2】EnvConfigPage 内置的前端版本 fallback 列表会烂掉
...
### E2.【P2】新增服务类型（如 PostgreSQL）的改动点清单
...
- "端口冲突检测与自动分配"（恢复 4.x）——恢复引擎无此实现（全库 grep 无 suggested_port/自动分配逻辑）；
- "后端日志英文化"——未达成（见 U5）；
...
| # | 事项 | 对应条目 | 预估 |
|---|---|---|---|
| 1 | 修复 zip-slip + 恶意包回归测试 | R1 | 2h |
| 2 | 新增 ci.yml（fmt/clippy/test/vitest/build） | Q1 | 2h |
| 3 | 补 LICENSE 文件 | Q3 | 5min |
| 4 | 修订 AGENTS.md、CHANGELOG（docs/README.md 版本头已随目录合并更新） | Q2 | 1h |
| 5 | 删除死代码（select_project_folder、@ts-ignore computed、无效测试转为真测试） | A6、Q4 | 2h |
| 6 | 修复多 PHP 服务自定义扩展串扰 | U1 | 0.5h |
...
|---|---|---|---|
| 1 | 恢复前自动备份（回滚包） | R2 | 2h |
| 2 | 恢复结果页展示明细 + 错误列表 | U2 | 2h |
| 3 | Docker 阻塞调用包 spawn_blocking | R3 | 2h |
| 4 | 容器状态枚举化（前后端契约） | R4 | 3h |
| 5 | 路径解析统一 + 用户配置迁至 app_data_dir | A1 | 4h |
| 6 | 备份流式写入 | R5 | 3h |
| 7 | 日志轮转 + UI 日志复制/清空/导出 | R6、U4 | 2h |
| 8 | Docker 不可用轮询退避 | U3 | 1h |

### 第三批：扩展与打磨（按需排期）

| # | 事项 | 对应条目 |
|---|---|---|
| 1 | 自动更新（tauri-plugin-updater 或最低配"检查更新"） | E5 |
| 2 | 版本清单外部覆盖 | E1 |
| 3 | manifest 版本兼容检查 | E4 |
| 4 | .env 解析去魔数切片 | A2 |
| 5 | 前端 api 层 + EnvConfigPage 服务面板配置化 | A3、E2 |
| 6 | i18n 收尾 + 主题适配收尾 + lint 基建 + CSP | U5、U6、Q5、Q6 |
| 7 | mysqldump / SQL 导入（维持低优先级） | E3 |
