...
impl MirrorSource {
    ...
    pub fn get_url(&self, service: &str) -> String {
        match (self, service) {
            // Docker Registry
            (MirrorSource::Ustc, "docker") => "https://docker.mirrors.ustc.edu.cn".to_string(),
            (MirrorSource::Tencent, "docker") => "https://mirror.ccs.tencentyun.com".to_string(),

            // APT
            (MirrorSource::Aliyun, "apt") => "http://mirrors.aliyun.com/debian/".to_string(),
            (MirrorSource::Tsinghua, "apt") => {
                "https://mirrors.tuna.tsinghua.edu.cn/debian/".to_string()
            }
            (MirrorSource::Ustc, "apt") => "https://mirrors.ustc.edu.cn/debian/".to_string(),
            ...
            // PyPI
            (MirrorSource::Aliyun, "pypi") => "https://mirrors.aliyun.com/pypi/simple/".to_string(),
            (MirrorSource::Tsinghua, "pypi") => {
                "https://pypi.tuna.tsinghua.edu.cn/simple".to_string()
            }

            // NPM
            (MirrorSource::Taobao, "npm") => "https://registry.npmmirror.com".to_string(),
            (MirrorSource::Tencent, "npm") => "https://mirrors.cloud.tencent.com/npm/".to_string(),
...
impl MirrorConfig {
    ...
    pub fn save_to_env(&self) -> Result<(), String> {
        ...
        env_content = Self::update_env_value(
            &env_content,
            "COMPOSER_MIRROR",
            self.composer_mirror.as_str(),
        );
        env_content =
            Self::update_env_value(&env_content, "PYPI_MIRROR", self.pypi_mirror.as_str());
...
",
                ...
                apt_url
                    .replace("debian", "debian-security")
                    .trim_end_matches('/')
            ));
        }
